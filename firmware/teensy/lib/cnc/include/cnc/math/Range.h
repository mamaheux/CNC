...
template<class T>
inline InclusiveRange3<T>::InclusiveRange3(const Vector3<T>& min, const Vector3<T>& max) : m_min(min),
                                                                                           m_max(max)
{
}
...
{
    return m_min.x <= v.x && m_min.y <= v.y && m_min.z <= v.z && v.x <= m_max.x && v.y <= m_max.y && v.z <= m_max.z;
}
