...
void Endstops::checkConfigErrors(std::function<void(const char*, const char*, const char*)> onMissingConfigItem) {
  ...
  CHECK_CONFIG_ERROR(onMissingConfigItem, m_xMaxConfig.has_value() || m_xRangeInMm.has_value(), X_MAX_PIN_KEY_OR_X_RANGE_IN_MM_KEY);
  CHECK_CONFIG_ERROR(onMissingConfigItem, m_yMaxConfig.has_value() || m_yRangeInMm.has_value(), Y_MAX_PIN_KEY_OR_Y_RANGE_IN_MM_KEY);
  CHECK_CONFIG_ERROR(onMissingConfigItem, m_zMaxConfig.has_value() || m_zRangeInMm.has_value(), Z_MAX_PIN_KEY_OR_Z_RANGE_IN_MM_KEY);
